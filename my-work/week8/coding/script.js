...
function gotData(incomingData){
  ...
  let graphGroup = viz.append("g").attr("class", "graphGroup");
	...
	data = [incomingData[0]];
	let datapoints = graphGroup.selectAll(".line").data(data, d=>d.year);
	//console.log(incomingData)
	let path = datapoints.enter()
		.append("path")
		.attr("class", "line")
		.attr("d", lineMaker)
		.attr("stroke-width", 5)
	...
	document.getElementById("usa").addEventListener("click", ()=>update(incomingData, 0));
	document.getElementById("china").addEventListener("click", ()=>update(incomingData, 1));
	
	function update(incomingData, k){
		data = [incomingData[k]];
		let datapoints = graphGroup.selectAll(".line").data(data, d=>d.year);
		datapoints
		.transition()
		.attr("d", lineMaker)
		.attr("stroke", (d)=>{if(d[0].country == "United States"){return "blue"}else{return "red"}});
