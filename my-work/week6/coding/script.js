...
function gotData(incomingData){
  ...
  function drawViz(){
	  ...
	  r2c = {"Asia": "red", "Europe": "yellow", "Africa": "brown", "Americas": "blue", "Oceania": "Green"};

    let currentYearData = incomingData.filter(filterYear);
    console.log("---\nthe currentYearData array now carries the data for year", currentYear, currentYearData);
    ...
    // bind currentYearData to elements
	  let datagroup = vizGroup.selectAll(".datagroup").data(currentYearData, d => d.Country);
	  


    // take care of entering elements
	  let entergroup = datagroup.enter()
	  	.append("g")
    ...
    // take care of updating elements
	  let exitgroup = datagroup.exit();
	  exitgroup.transition().attr("opacity", 0).remove();
	  
	  
	  datagroup.transition().attr("transform", d => "translate("+ xScale(d.fert) +","+ yScale(d.life) +")");
